...
function AdminAddEmployee({ team }) {
        ...
        <Card className="h-full w-full">
                    ...
                    <DialogBody divider className="grid place-items-center gap-4">
                        <div className="mb-2 flex flex-col gap-3 w-full">
                          ...
                          <Input color='green' size="lg" label="Position"
                            value={data.position ?? ''} onChange={e => setData('position', e.target.value)} error={errors.position} />
          ...
          <CardBody className="overflow-scroll px-0">
            <table className="w-full min-w-max table-auto text-left">
              <thead>
                <tr>
                  {['Image', 'Name', 'Position', 'Description', ''].map((head) => (
              ...
              <tbody>
                {team && team.map(({ id, image, position, name, description }, index) => {
